...
static int qlsInit = 1;
  ...
  BinData::BinData() {
    ...
    this->slsb = new LineSeriesBuilderSimple();
    this->lsb = new LineSeriesBuilderMinMax();
    this->sfulsb = new LineSeriesBuilderSimpleFillUnder();
    this->fulsb = new LineSeriesBuilderMinMaxFillUnder();

  }

  BinData::~BinData() {

    if ( this->slsb ) delete this->slsb; this->slsb = nullptr;
    if ( lsb ) delete lsb; lsb = nullptr;
    if ( fulsb ) delete fulsb; fulsb = nullptr;
    if ( sfulsb ) delete sfulsb; sfulsb = nullptr;
  ...
  int BinData::genLineSeries( QString filename,
    ...
    //std::cout << "num sigs: " << numSigs << std::endl;
    ...
    //std::cout << "numSigbytes 2: " << numSigbytes << std::endl;
    //std::cout << "offset 1: " << offset << std::endl;
    ...
    if ( totalpoints < ( plotAreaWidthPixels * 5 ) ) { // build simple line series
      ...
      for (iread = 0; iread < numReadOps; iread++) {
        ...
        updateLineSeries(iread, pts, slope, intercept, timeStep, plotAreaWidthPixels, startTimeInSec, endTimeInSec,
                         dataTimeIncrementInSec, numBytesRead, buf, slsb, miny, maxy);
      ...
      for (iread = 0; iread < numReadOps; iread++) {
        ...
        numBytesRead = readTraceData(fb, buf, 4000);
        updateLineSeries(iread, pts, slope, intercept, timeStep, plotAreaWidthPixels, startTimeInSec, endTimeInSec,
                         dataTimeIncrementInSec, numBytesRead, buf, lsb, miny, maxy);
        for ( i=0; ( i<1000 && ifft<(maxFft-1) ); i++, ifft++ ) {
          fftArray[ifft][0] = slope * (double) buf[i] + intercept;
          fftArray[ifft][1] = 0.0;
        }
